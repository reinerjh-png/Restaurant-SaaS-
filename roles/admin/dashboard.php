    ...
    <div class="main-content">
        <div class="page-content">
            ...
            <div class="card mb-24">
                <div class="card-title">💳 Ingresos por método de pago — hoy</div>
                <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(130px,1fr));gap:12px;">
                    ...
                    <div style="background:var(--fondo);border-radius:10px;padding:14px 12px;text-align:center;border:1px solid var(--borde);">
                        <div style="font-size:1.6rem;"><?= $icon ?></div>
                        <div style="font-weight:800;font-size:1.1rem;margin:4px 0;">S/ <?= number_format($metodoMap[$key] ?? 0, 2) ?></div>
                        <div style="font-size:.75rem;color:var(--texto-light);font-weight:500;"><?= $label ?></div>
                    </div>
                    <?php endforeach; ?>
                </div>
            </div>
                ...
                <div class="card">
                    ...
                    <div style="display:flex;flex-direction:column;gap:10px;">
                        ...
                        <div style="display:flex;align-items:center;gap:10px;">
                            <div style="width:26px;height:26px;background:<?= $i===0?'var(--dorado)':($i===1?'var(--gris-claro)':'var(--borde)') ?>;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:.75rem;font-weight:800;color:<?= $i<2?'var(--texto)':'var(--texto-light)' ?>;"><?= $i+1 ?></div>
                            <div style="flex:1;">
                                <div style="font-size:.85rem;font-weight:600;"><?= htmlspecialchars($p['nombre']) ?></div>
                                <div style="font-size:.73rem;color:var(--texto-light);"><?= $p['vendidos'] ?> vendidos</div>
                            </div>
                            <div style="font-size:.85rem;font-weight:700;color:var(--rojo);">S/ <?= number_format($p['ingreso'],2) ?></div>
                        </div>
                        <?php endforeach; ?>
                    </div>
                    <?php else: ?>
                    <div class="empty-state" style="padding:30px 0;"><div class="icon">🍽️</div><p>Sin ventas hoy aún</p></div>
                    <?php endif; ?>
                </div>

                <!-- Últimos pedidos -->
                <div class="card">
                    <div class="card-title">🗂️ Últimos pedidos</div>
                    ...
                    <div style="display:flex;flex-direction:column;gap:8px;">
                        ...
                        <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--gris-claro);">
                            <span style="font-size:1rem;"><?= $p['tipo']==='aqui'?'🏠':'🛍️' ?></span>
                            <div style="flex:1;">
                                <div style="font-size:.82rem;font-weight:600;">
                                    <?= $p['mesa_numero'] ? 'Mesa '.$p['mesa_numero'] : 'Para Llevar' ?>
                                </div>
                                <div style="font-size:.7rem;color:var(--texto-light);"><?= htmlspecialchars($p['cajero']) ?> · <?= date('H:i', strtotime($p['created_at'])) ?></div>
                            </div>
                            <div style="font-weight:700;font-size:.88rem;color:var(--exito);">S/ <?= number_format($p['total'],2) ?></div>
                        </div>
                        <?php endforeach; ?>
                    </div>
                    <?php else: ?>
                    <div class="empty-state" style="padding:30px 0;"><div class="icon">🧾</div><p>Sin pedidos cobrados hoy</p></div>
                    <?php endif; ?>
                </div>
            </div>
